...
//
// DESCRIPTION
// 	implementation file for the main drawing view of the application.
//
// COPYRIGHT
...
- (void) buttonReleasedEvent:(WacomStylusEvent *)stylusEvent_I viewController:(UIViewController *) aViewController;
- (void) buttonPressedEvent:(WacomStylusEvent *)stylusEvent_I viewController:(UIViewController *) aViewController;
...
- (id) initWithFrame:(CGRect)frame_I
{
...
- (id) initWithCoder:(NSCoder*)coder_I
{
	if ((self = [super initWithCoder:coder_I]))
	{
...
- (void) setupBrush:(CGFloat)size_I
{
...
- (double) calcBrushSize:(double)pressure_I
{
	long maximumPressure = [[[WacomManager getManager] getSelectedDevice] getMaximumPressure];
	double brushSize = MIN_BRUSH_SIZE;
	if (pressure_I > 0.0)
	{
		double scale = (pressure_I < THRESHOLD ? maximumPressure : (double)(((maximumPressure - THRESHOLD) + 1.0)) / (pressure_I - THRESHOLD));
...
- (void) displayBuffer:(CADisplayLink *) __unused displayLink_I
{
	glBindRenderbufferOES(GL_RENDERBUFFER_OES, viewRenderbuffer);
...
- (void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event_I
{
...
- (void) touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event_I
{
	[self drawTouches:touches withEvent:event_I];
}
...
- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event_I
{
	[self drawTouches:touches withEvent:event_I];
}
...
- (void) buttonReleasedEvent:(WacomStylusEvent *)stylusEvent_I viewController:(UIViewController *) aViewController
{
	...
	switch ([stylusEvent_I getButton])
	{
		case 2:
		{
	...
	if (message != nil)
	{
		...
		if (alert == nil)
		{
			[aViewController presentViewController:alert1 animated:YES completion:^(void) {}];
		}
	}
}
...
- (void) buttonPressedEvent:(WacomStylusEvent *)stylusEvent_I viewController:(UIViewController *) aViewController
{
	...
	if (message != nil)
	{
		...
		[aViewController presentViewController:alert animated:YES completion:^(void) { }];
	}
...
- (void) stylusEvent:(WacomStylusEvent *)stylusEvent_I
{
	...
	switch ([stylusEvent_I getType])
	{
		case eStylusEventType_PressureChange:
		{
			mPressure = [stylusEvent_I getPressure];
			break;
		}
		case eStylusEventType_ButtonReleased:
		{
			[self buttonReleasedEvent: stylusEvent_I viewController: rootViewController];
			break;
		}
			
		case eStylusEventType_ButtonPressed:
		{
			[self buttonPressedEvent: stylusEvent_I viewController: rootViewController];
			break;
		}
